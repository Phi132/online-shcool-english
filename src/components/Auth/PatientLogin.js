...
class PatientLogin extends Component {
    ...
    onSubmitForm = async (e) => {
        e.preventDefault();
        const { patientLoginSuccess, processLogout } = this.props;
        try {
            var server = await patientLogin(this.state.username, this.state.password)
            // console.log(server.data.checkLoginUserHomeamess);
            if (server.data.checkLoginUserHomea.err !== 0) {
                this.setState({
                    errorMess: server.data.checkLoginUserHomea.mess
                })
            }
